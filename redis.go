...
func (redis *Redis) get(key string, z *Zone) *Record {
	...
	if err != nil {
		fmt.Println("error getting HGET: ", err, redis.keyPrefix+z.Name+redis.keySuffix, label)
		return nil
	}
...
func (redis *Redis) Connect() {
	redis.Pool = &redisCon.Pool{
		Dial: func() (redisCon.Conn, error) {
			...
			if redis.tlsSkipVerify {
				opts = append(opts, redisCon.DialTLSSkipVerify(redis.tlsSkipVerify))
			}
...
func (redis *Redis) load(zone string) *Zone {
	...
	if err != nil {
		fmt.Println("error getting HKEYS: ", err, redis.keyPrefix+zone+redis.keySuffix)
		return nil
	}
	z := new(Zone)
	z.Name = zone
